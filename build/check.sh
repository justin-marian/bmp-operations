...
function check_valgrind {
    ...
    mkdir -p ${TESTS_OUT}/task7/valgrind_results # Create the directory for Valgrind results
    ...
    do
        ...
        # Redirect Valgrind output to a specific file
        valgrind_output_file="${TESTS_OUT}/task7/valgrind_results/valgrind_result_test${test_id}.txt"
        valgrind --tool=memcheck --leak-check=full --error-exitcode=1 ./interactive < ${in_file} > /dev/null 2> $valgrind_output_file
        ...
        then
            ...
            print_result "Test ${test_id}" "0/${test_score}p failed"
            echo "Valgrind detected issues. Check ${valgrind_output_file} for details."
        fi
    done
}
...
function check_style {
    ...
    cpplint --filter=-legal/copyright,-readability/casting,-build/include_subdir,-runtime/threadsafe_fn,-build/header_guard,-runtime/int \
        --linelength=120 $( find ../include/ -name \*.h | grep -v -e "pattern_to_exclude" ) $( find ../src/ -name \*.c | grep -v -e "check16.c" -e "bmp.c" ) &>> result
    ...
    echo -e "\nRun clang-tidy\n" >> result
    clang-tidy -checks='-*,cppcoreguidelines*' --warnings-as-errors='-*,cppcoreguidelines*' $( find ../include/ -name \*.h | grep -v -e "pattern_to_exclude" ) $( find ../src/ -name \*.c | grep -v -e "check16.c" -e "bmp.c" ) -- &>> result
